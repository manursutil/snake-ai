...
class SnakeEnv():
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        danger_straight = 1 if self._danger_at(state, head_x + forward[0], head_y + forward[1]) else 0
        danger_left     = 1 if self._danger_at(state, head_x + left[0], head_y + left[1]) else 0
        danger_right    = 1 if self._danger_at(state, head_x + right[0], head_y + right[1]) else 0
    ...
    def _danger_at(self, state, x, y):
        if x < 0 or x >= SnakeEnv.WIDTH // SnakeEnv.CELL or y < 0 or y >= SnakeEnv.HEIGHT // SnakeEnv.CELL:
            return True
